...
server's browser, and running the capture stack locally. (Land.com's official
feed API was evaluated and rejected — see the decision note in `specs/specs.md`.)

**Deliverable:** `research/land-com-connect-spike/report.md` comparing the
...
- Establish the exact scope of the Akamai block with fresh evidence: which
  land.com hosts/paths are blocked from the worker box, and confirm the
  operator's home IP is not blocked.
...
- **OT-A:** From a home/residential connection, run the probe commands US-002
  prepares (curl land.com login/home endpoints) and paste the output
...
**Acceptance Criteria:**
- [x] `research/land-com-connect-spike/report.md` created with section headings: The Block, Block Scope, Capture From the User's Browser, Residential Egress, Run Locally, Comparison & Recommendation (a feed-API heading existed at the time; that option was later rejected — see `specs/specs.md`)
...
are blocked from the server, so every access option is judged against the block's
real scope.

**Acceptance Criteria:**
- [x] From the worker box, record HTTP status + response size (no bodies beyond the first line) for: `https://www.land.com/`, `https://www.land.com/login`, `https://www.landsofamerica.com/`, and `https://www.landwatch.com/` — all in a Block Scope table with fetch timestamps
- [x] Block Scope section states the conclusion explicitly: which land.com surfaces are blocked from the datacenter IP
...
### US-006: Removed
Removed: the Land.com feed-API option was rejected — see the decision note in `specs/specs.md`.
...
- [x] A "what would change this decision" list (e.g. Akamai starts blocking the home IP, land.com adds MFA)
...
- **No bot-evasion tooling** — no fingerprint spoofing, stealth plugins, or
  CAPTCHA-solving services. Options must be ordinary legitimate access from an
  IP Land.com serves (the operator's own).
...
- Build on, don't repeat: `research/auth-capture-spike/initial-research.md`
  (§1 extension pattern) and `research/auth-capture-spike/report.md`. Cite;
  only re-verify what the block could have changed.
