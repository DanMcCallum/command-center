...
**Follow-up implementation PRD is drafted inside the report (§7):** Ralph-sized stories US-201–203 (Browserbase live-view path), plus an operator-task prerequisite (create a Browserbase account) called out as *not* Ralph work. That section is the ready-to-run spec if the proposal is accepted — it is not yet its own `specs/` PRD.
